...
$vroot2forums  = array();
$forumid2name  = array(
    0 => "<img src=\"{$PHORUM['http_path']}/mods/postcount_tagging/settings/folder.gif\" style=\"border:0\"/>&nbsp;Top level forum folder"
);
foreach ($forums as $forum)
{
    if ($forum['forum_id'] == $forum['vroot'])
    {
        $forumid2name[$forum['forum_id']]  =
            "<img src=\"{$PHORUM['http_path']}/mods/postcount_tagging/settings/folder.gif\" style=\"border:0\"/>&nbsp;" .
...
foreach ($vroot2forums as $vroot => $forums) {
    foreach ($forums as $fid => $forum) {
        ...
        $forumid2name[$fid] = "<img src=\"{$PHORUM['http_path']}/mods/postcount_tagging/settings/forum.gif\" style=\"border:0\"/>&nbsp;$name";
        array_pop($path);
        $name = strip_tags(implode("::", array_reverse($path)));
...
$ignores = postcount_tagging_get_ignore_list();
...
if (isset($_POST['delete_id']) || isset($_GET['delete_id'])) {
    ...
    unset($ignores[$id]);
    postcount_tagging_store_ignore_list($ignores);
}
if (isset($_POST['add'])) {
...
<div class="PhorumAdminTitle">
  List of ignored vroots and forums
</div>
...
else foreach ($ignores as $id)
{
    ...
    $delete_url = $base_url . '&list_ignore=1&delete_id=' . $id;
    ?>
    <tr>
